...
public class AssetServiceImpl implements AssetService {
	...
	private void initializeAssetHierarchy(AssetEquipment assetInfor, Equipment assetParam, InforContext context) {
		...
		if (tools.isEmpty(assetParam.getHierarchyAssetDependent())) {
			assetParam.setHierarchyAssetDependent("FALSE");
		}

		if (tools.isEmpty(assetParam.getHierarchyPositionDependent())) {
			assetParam.setHierarchyPositionDependent("FALSE");
		}

		if (tools.isEmpty(assetParam.getHierarchyPrimarySystemDependent())) {
		...
		if (tools.getDataTypeTools().isTrueValue(assetParam.getHierarchyAssetDependent())) {
			...
			// Non dependent position
			if (!tools.isEmpty(assetParam.getHierarchyPositionCode())) {
				assetParentHierarchy.getAssetDependency()
						.setNONDEPENDENTPOSITION(this.createHierarchyPosition(assetParam, hierarchyPosition));
			}

			// Non dependent system
			if (!tools.isEmpty(assetParam.getHierarchyPrimarySystemCode())) {
				assetParentHierarchy.getAssetDependency()
		...
		// Position dependent
			...
			// Non dependent asset
			if (!tools.isEmpty(assetParam.getHierarchyAssetCode())) {
